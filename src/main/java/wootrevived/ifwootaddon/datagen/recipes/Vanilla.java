...
public class Vanilla {
    public static void registerRecipes(Recipes recipes, Consumer<FinishedRecipe> consumer){
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, IFItemsRegistry.STYGIAN_LASER_LENS_ITEM.get(), 1)
                .requires(Ingredient.of(Arrays.stream(ModuleCore.LASER_LENS).map(RegistryObject::get).map(Item::getDefaultInstance)))
                ...
                .save(consumer, IFWootAddon.location("laser_lens_all"));
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, IFItemsRegistry.STYGIAN_LASER_LENS_ITEM.get(), 1)
                ...
                .save(consumer, IFWootAddon.location("laser_lens_self"));
        ...
        for(int i = 0; i < ModuleCore.LASER_LENS.length; i++){
            ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, ModuleCore.LASER_LENS[i].get(), 1)
                    ...
                    .save(consumer, IFWootAddon.location("laser_lens_" + DyeColor.byId(i).getName()));
        }
    }
}
